...
class BestScore(AIEngine):
    ...
    async def next_move(self, state):
        ...
        min_action_scores = [(action, await self._find_min_score_from_all_possibilities(state, new_tile_value=2))
                             for action, state in action_states]
        # print('min_action_scores ', [(action, int(new_score) - int(state['score']))
        #                              for action, new_score in min_action_scores])
        max_action_score = max(min_action_scores, key=lambda a_s: a_s[1])
